...
layui.use(["element", "form", "layer", "okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday", "laydate"], function () {
    ...
    const url_user = "data/navs.json"
    const url_admin = "data/navs1.json"
    const type = coreUtil.getLocalData("type")
    let url = undefined
    if(0===parseInt(type)){
        url = url_user
    }else{
    ...
    function lockShowInit(okUtils) {
        ...
        $("#lockQuit").click(function () {
            ...
            window.location.replace("/logout");  //替换当前页面
        });
    }

    console.log("加载完成okadmin.js");
});
